import { collection, addDoc, getDocs, doc, updateDoc, deleteDoc, Timestamp, getDoc, query, where } from 'firebase/firestore';
...
export class CampaignService {
  ...
  // Delete campaign AND all customer assignments
  static async deleteCampaign(campaignId) {
    try {
      const user = auth.currentUser;
      if (!user) throw new Error('No authenticated user');

      console.log('🗑️ Deleting campaign and all customer assignments:', campaignId);
      ...
      // Step 2: Delete the main campaign
      await deleteDoc(doc(firestore, 'users', user.uid, 'campaigns', campaignId));
      console.log('✅ Main campaign deleted');

      // Step 3: Delete all customer assignments (promotions generated by this campaign)
      ...
      for (const customerDoc of customersSnapshot.docs) {
        ...
        const promotionsByNameSnapshot = await getDocs(promotionsByNameQuery);
        
        // Delete promotions found by campaignId
        for (const promotionDoc of promotionsSnapshot.docs) {
          await deleteDoc(promotionDoc.ref);
          deletedAssignments++;
          console.log(`✅ Deleted campaign assignment for customer: ${customerId} (by campaignId)`);
        }
        
        // Delete promotions found by campaignName (if not already deleted)
        for (const promotionDoc of promotionsByNameSnapshot.docs) {
          // Check if this promotion wasn't already deleted
          const alreadyDeleted = promotionsSnapshot.docs.some(doc => doc.id === promotionDoc.id);
          if (!alreadyDeleted) {
            await deleteDoc(promotionDoc.ref);
            deletedAssignments++;
            console.log(`✅ Deleted campaign assignment for customer: ${customerId} (by campaignName)`);
          }
        }
      }

      console.log(`✅ Campaign deletion complete! Deleted ${deletedAssignments} customer assignments`);
    } catch (error) {
